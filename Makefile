...
install-test: ## Run installer smoke test in temp HOME
	...
	if [ -f "$$TMP_HOME/.config/opencode/my_opencode/scripts/hotfix_command.py" ]; then HOTFIX_REPO="$$TMP_HOME/.config/opencode/my_opencode"; (cd "$$HOTFIX_REPO" && HOME="$$TMP_HOME" python3 "$$HOTFIX_REPO/scripts/hotfix_command.py" start --incident-id INSTALL-TEST-1 --scope config_only --impact sev3 --json); (cd "$$HOTFIX_REPO" && HOME="$$TMP_HOME" python3 "$$HOTFIX_REPO/scripts/hotfix_runtime.py" checkpoint --label install-test --json); (cd "$$HOTFIX_REPO" && HOME="$$TMP_HOME" python3 "$$HOTFIX_REPO/scripts/hotfix_runtime.py" validate --target validate --result pass --json); (cd "$$HOTFIX_REPO" && HOME="$$TMP_HOME" python3 "$$HOTFIX_REPO/scripts/hotfix_command.py" status --json); (cd "$$HOTFIX_REPO" && HOME="$$TMP_HOME" python3 "$$HOTFIX_REPO/scripts/hotfix_command.py" remind --json); (cd "$$HOTFIX_REPO" && HOME="$$TMP_HOME" python3 "$$HOTFIX_REPO/scripts/hotfix_command.py" close --outcome resolved --followup-issue install-test-followup --deferred-validation-owner oncall --deferred-validation-due 2026-03-01 --json); (cd "$$HOTFIX_REPO" && HOME="$$TMP_HOME" python3 "$$HOTFIX_REPO/scripts/hotfix_command.py" doctor --json); fi; \
