...
PYTHON_TARGETS := validate build-agents build-agents-check release-index-update docs-automation-summary-update docs-automation-check pages-readiness-check release-note-validation-check release-note-quality-check plan-hygiene-check wave-linkage-check wave-handoff-summary wave-completion-update quality-fast quality-strict quality-off quality-status gateway-status gateway-enable gateway-disable gateway-doctor gateway-secret-redaction-smoke gateway-turn-watch gateway-turn-watch-webhook harness-wave2-task4-smoke notify-icons-generate notify-icons-select reservation-status selftest doctor doctor-json devtools-status hooks-install install-test release-check release
...
install-test: ## Run installer smoke test in temp HOME
	...
	if [ -f "$$TMP_HOME/.config/opencode/my_opencode/scripts/hotfix_command.py" ]; then HOTFIX_REPO="$$TMP_HOME/.config/opencode/my_opencode"; (cd "$$HOTFIX_REPO" && HOME="$$TMP_HOME" $(PYTHON) "$$HOTFIX_REPO/scripts/hotfix_command.py" start --incident-id INSTALL-TEST-1 --scope rollback --impact sev2 --json); (cd "$$HOTFIX_REPO" && HOME="$$TMP_HOME" $(PYTHON) "$$HOTFIX_REPO/scripts/hotfix_runtime.py" checkpoint --label install-test --json); (cd "$$HOTFIX_REPO" && HOME="$$TMP_HOME" $(PYTHON) "$$HOTFIX_REPO/scripts/hotfix_runtime.py" mark-patch --summary "rollback to stable state" --json); (cd "$$HOTFIX_REPO" && HOME="$$TMP_HOME" $(PYTHON) "$$HOTFIX_REPO/scripts/hotfix_runtime.py" validate --target validate --result pass --json); (cd "$$HOTFIX_REPO" && HOME="$$TMP_HOME" $(PYTHON) "$$HOTFIX_REPO/scripts/hotfix_command.py" status --json); (cd "$$HOTFIX_REPO" && HOME="$$TMP_HOME" $(PYTHON) "$$HOTFIX_REPO/scripts/hotfix_command.py" remind --json); (cd "$$HOTFIX_REPO" && if HOME="$$TMP_HOME" $(PYTHON) "$$HOTFIX_REPO/scripts/hotfix_command.py" close --outcome rolled_back --json; then echo "hotfix close missing followup unexpectedly passed" && exit 1; fi); (cd "$$HOTFIX_REPO" && HOME="$$TMP_HOME" $(PYTHON) "$$HOTFIX_REPO/scripts/hotfix_command.py" close --outcome rolled_back --followup-issue install-test-followup --deferred-validation-owner oncall --deferred-validation-due 2026-03-01 --json); (cd "$$HOTFIX_REPO" && HOME="$$TMP_HOME" $(PYTHON) "$$HOTFIX_REPO/scripts/hotfix_command.py" doctor --json); fi; \
