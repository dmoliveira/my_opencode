...
| `tasker` | primary | planning-focused task/epic/dependency/note capture and bounded research synthesis through Codememory | contract-only | [h:648c76a8]
...
- `tasker` is the preferred primary for Codememory-backed planning capture, backlog shaping, dependency mapping, bounded read-only research synthesis, and durable implementation notes when no code execution should occur. [h:5215ff93]
...
- `tasker` may use no more than two total read-only `explore`/`librarian` research delegates per user request, must synthesize their evidence before persistence, and must never delegate implementation or validation. [h:newa2a04a]
