...
import {
  ...
  type GatewayLlmDecisionMode,
} from "../llm-decision-bindings.js";
import {
...
function exactHookIdentityList(value: unknown, label: string): string[] {
  if (value === undefined) {
    return [];
  }
  if (!Array.isArray(value)) {
...
function durationThreshold(value: unknown, fallback: string): string {
  ...
  return /^(?:\d+)\s*(?:s|sec|secs|second|seconds|m|min|mins|minute|minutes|h|hr|hrs|hour|hours|d|day|days)$/.test(
    trimmed,
  )
    ? trimmed
    : fallback;
...
function llmDecisionMode(
  value: unknown,
  fallback: GatewayLlmDecisionMode,
  label = "llmDecisionRuntime.mode",
): GatewayLlmDecisionMode {
  if (value === undefined) {
    return fallback;
  }
  if (typeof value === "string" && LLM_DECISION_MODES.has(value)) {
    return value as GatewayLlmDecisionMode;
  }
  throw new Error(
    `${label} must be one of: ${GATEWAY_LLM_DECISION_MODES.join(", ")}`,
  );
}
...
function llmDecisionHookModes(
  value: unknown,
): Record<string, GatewayLlmDecisionMode> {
  if (value === undefined) {
    return {};
  }
  if (!isRecord(value)) {
  ...
  const result: Record<string, GatewayLlmDecisionMode> = {};
  for (const [key, raw] of Object.entries(value)) {
    if (!LLM_DECISION_HOOK_IDS.has(key)) {
      throw new Error(
        `llmDecisionRuntime.hookModes contains unknown LLM hook id: ${key}`,
      );
    }
    result[key] = llmDecisionMode(
      raw,
      "disabled",
      `llmDecisionRuntime.hookModes.${key}`,
    );
  }
  return result;
}
...
export function loadGatewayConfig(raw: unknown): GatewayConfig {
  ...
  const contextInjectorSource =
    source.contextInjector && typeof source.contextInjector === "object"
  ...
  const globalProcessPressureSource =
    ...
    typeof source.globalProcessPressure === "object"
      ...
      : {};
  const longTurnWatchdogSource =
    source.longTurnWatchdog && typeof source.longTurnWatchdog === "object"
      ? (source.longTurnWatchdog as Record<string, unknown>)
      : {};
  const notifyEventsSource =
    source.notifyEvents && typeof source.notifyEvents === "object"
      ? (source.notifyEvents as Record<string, unknown>)
      : {};
  ...
  const conciseModeSource =
    source.conciseMode && typeof source.conciseMode === "object"
      ...
      : {};
  const executionStatusSource =
    source.executionStatus && typeof source.executionStatus === "object"
      ? (source.executionStatus as Record<string, unknown>)
      : {};
  ...
  const adaptiveDelegationPolicySource =
    ...
    typeof source.adaptiveDelegationPolicy === "object"
      ...
      : {};
  const llmDecisionRuntimeSource = optionalConfigRecord(
  ...
  const validationEvidenceLedgerSource =
    ...
    typeof source.validationEvidenceLedger === "object"
      ...
      : {};
  const mistakeLedgerSource =
    source.mistakeLedger && typeof source.mistakeLedger === "object"
      ? (source.mistakeLedger as Record<string, unknown>)
      : {};
  ...
  const jsonErrorRecoverySource =
    source.jsonErrorRecovery && typeof source.jsonErrorRecovery === "object"
      ...
      : {};
  const providerTokenLimitRecoverySource =
  ...
  const modeTransitionReminderSource =
    ...
    typeof source.modeTransitionReminder === "object"
      ...
      : {};
  const todoreadCadenceReminderSource =
  ...
  const hookOrder = semanticSummarizerHookOrder(
    exactHookIdentityList(hooksSource.order, "hooks.order"),
  ...
  return {
    ...
    globalProcessPressure: {
      ...
      criticalReminderCooldownToolCalls: positiveInt(
        ...
        DEFAULT_GATEWAY_CONFIG.globalProcessPressure
    ...
    longTurnWatchdog: {
      ...
      maxSessionStateEntries: positiveInt(
        longTurnWatchdogSource.maxSessionStateEntries,
        DEFAULT_GATEWAY_CONFIG.longTurnWatchdog.maxSessionStateEntries,
      ),
      prefix:
        typeof longTurnWatchdogSource.prefix === "string" &&
    ...
    notifyEvents: {
      enabled:
        typeof notifyEventsSource.enabled === "boolean"
          ? notifyEventsSource.enabled
          : DEFAULT_GATEWAY_CONFIG.notifyEvents.enabled,
      cooldownMs: nonNegativeInt(
        notifyEventsSource.cooldownMs,
        DEFAULT_GATEWAY_CONFIG.notifyEvents.cooldownMs,
      ),
      style:
        notifyEventsSource.style === "detailed"
          ? "detailed"
    ...
    pressureEscalationGuard: {
      ...
      blockedSubagentTypes:
        stringList(pressureEscalationGuardSource.blockedSubagentTypes).length >
        0
    ...
    sessionRuntimeSystemContext: {
      ...
      injectSessionIdContext:
        typeof sessionRuntimeSystemContextSource.injectSessionIdContext ===
        "boolean"
      ...
      injectSessionIdWhenConciseModeOnly:
        typeof sessionRuntimeSystemContextSource.injectSessionIdWhenConciseModeOnly ===
        "boolean"
    ...
    executionStatus: {
      enabled:
        typeof executionStatusSource.enabled === "boolean"
          ? executionStatusSource.enabled
          : DEFAULT_GATEWAY_CONFIG.executionStatus.enabled,
      maxSessions: boundedInteger(
        executionStatusSource.maxSessions,
        1,
        64,
        DEFAULT_GATEWAY_CONFIG.executionStatus.maxSessions,
      ),
      maxLabelChars: boundedInteger(
        executionStatusSource.maxLabelChars,
        24,
        160,
        DEFAULT_GATEWAY_CONFIG.executionStatus.maxLabelChars,
      ),
    },
    delegateTaskRetry: {
      enabled:
    ...
    mistakeLedger: {
      enabled:
        typeof mistakeLedgerSource.enabled === "boolean"
          ? mistakeLedgerSource.enabled
          : DEFAULT_GATEWAY_CONFIG.mistakeLedger.enabled,
      path:
        typeof mistakeLedgerSource.path === "string" &&
    ...
    adaptiveValidationScheduler: {
      ...
      reminderEditThreshold: nonNegativeInt(
        ...
        DEFAULT_GATEWAY_CONFIG.adaptiveValidationScheduler
    ...
    agentReservationGuard: {
      ...
      stateFile:
        typeof agentReservationSource.stateFile === "string" &&
    ...
    providerTokenLimitRecovery: {
      enabled:
        typeof providerTokenLimitRecoverySource.enabled === "boolean"
    ...
    todoreadCadenceReminder: {
      enabled:
        typeof todoreadCadenceReminderSource.enabled === "boolean"
    ...
    workflowConformanceGuard: {
      ...
      blockEditsOnProtectedBranches:
        typeof workflowConformanceSource.blockEditsOnProtectedBranches ===
        "boolean"
    ...
    branchFreshnessGuard: {
      ...
      baseRef:
        typeof branchFreshnessSource.baseRef === "string" &&
      ...
      activeReservationPathsEnvKeys:
        parallelWriterConflictSource.activeReservationPathsEnvKeys === undefined
          ? DEFAULT_GATEWAY_CONFIG.parallelWriterConflictGuard
              .activeReservationPathsEnvKeys
          : stringList(
              parallelWriterConflictSource.activeReservationPathsEnvKeys,
            ),
      enforceReservationCoverage:
        typeof parallelWriterConflictSource.enforceReservationCoverage ===
        "boolean"
          ? parallelWriterConflictSource.enforceReservationCoverage
          : DEFAULT_GATEWAY_CONFIG.parallelWriterConflictGuard
              .enforceReservationCoverage,
      stateFile:
        typeof parallelWriterConflictSource.stateFile === "string" &&
    ...
    quality: {
      ...
      ts: {
        ...
        typecheck:
          typeof tsSource.typecheck === "boolean" ? tsSource.typecheck : true,
        tests: typeof tsSource.tests === "boolean" ? tsSource.tests : false,
      },
      py: {
        selftest:
          typeof pySource.selftest === "boolean" ? pySource.selftest : true,
      },
    },
  };
}
