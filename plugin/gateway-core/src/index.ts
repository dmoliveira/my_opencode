...
} from "./hooks/shared/delegation-child-session.js";
...
function configuredHooks(
  ...
  writeGatewayEventAudit(directory, {
    ...
    sidecar_layers: loadedConfig.meta.layers
      .map((layer) =>
        `${layer.kind}:${layer.exists ? (layer.loaded ? "loaded" : "invalid") : "missing"}`,
      )
      .join(","),
  ...
  const hooks: Array<GatewayHook | null> = [
    ...
    safeHook("direct-work-warning", () =>
      createDirectWorkWarningHook({
        directory,
...
export default function GatewayCorePlugin(
  ...
  async function event(input: GatewayEventPayload): Promise<void> {
    ...
    const terminalChildSessionId = delegationTerminalChildSessionId(
      eventType,
      payload,
    );
