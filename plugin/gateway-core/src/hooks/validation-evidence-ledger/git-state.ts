...
const FINGERPRINT_VERSION = "git-state-v1" as const
...
function resolveContainedPath(root: string, pathValue: string): string {
...
function readRegularFileNoFollow(path: string, expectedSize: number): Buffer {
...
function untrackedStateDigest(root: string, trackedDiff: Buffer): string {
  ...
  for (const pathBytes of untracked) {
    ...
    if (
      content.length > MAX_UNTRACKED_FILE_BYTES ||
...
export function captureGitStateFingerprint(directory: string): GitStateFingerprint | null {
...
export function sameGitState(
  left: GitStateFingerprint | null | undefined,
  right: GitStateFingerprint | null | undefined,
): boolean {
  return Boolean(
    left &&
      right &&
