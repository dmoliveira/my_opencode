...
function getSessionState(store: Map<string, SessionState>, sessionId: string): SessionState {
  ...
  const created: SessionState = { startReminderSent: false, eventsSinceReminder: 0 }
  store.set(sessionId, created)
  return created
}

function hasStartSignal(output: string): boolean {
  return START_PATTERNS.some((pattern) => pattern.test(output))
}

function hasCheckpointSignal(output: string): boolean {
