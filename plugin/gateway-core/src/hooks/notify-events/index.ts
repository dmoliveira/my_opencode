...
function parseState(raw: unknown): NotifyState {
  ...
  if (source.sound && typeof source.sound === "object") {
    ...
    if (sound.eventThemes && typeof sound.eventThemes === "object") {
      for (const event of Object.keys(state.sound.eventThemes) as NotifyEvent[]) {
        const themeOverride = normalizeLabel((sound.eventThemes as Record<string, unknown>)[event]).toLowerCase()
        if (themeOverride) {
    ...
    if (sound.customFiles && typeof sound.customFiles === "object") {
      for (const event of Object.keys(state.sound.customFiles) as NotifyEvent[]) {
        const custom = normalizeLabel((sound.customFiles as Record<string, unknown>)[event])
        if (custom || (sound.customFiles as Record<string, unknown>)[event] === "") {
  ...
  if (source.channels && typeof source.channels === "object") {
    for (const key of Object.keys(state.channels) as NotifyEvent[]) {
      ...
      state.channels[key].sound = toBool((channels as Record<string, unknown>).sound, state.channels[key].sound)
      state.channels[key].visual = toBool((channels as Record<string, unknown>).visual, state.channels[key].visual)
    }
  }
  return state
}
...
function titleWithIcon(eventName: NotifyEvent, state: NotifyState, title: string): string {
...
function iconImagePath(eventName: NotifyEvent, state: NotifyState, directory: string): string {
  if (!state.icons.enabled) {
    return ""
  }
...
function resolvedEventTheme(state: NotifyState, eventName: NotifyEvent): string {
...
function customSoundPath(state: NotifyState, eventName: NotifyEvent, directory: string): string {
...
function notifyVisualLinux(title: string, message: string, imagePath: string): boolean {
...
function notifySound(eventName: NotifyEvent, state: NotifyState, directory: string): boolean {
