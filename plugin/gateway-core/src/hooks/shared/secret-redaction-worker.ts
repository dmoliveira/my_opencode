...
import {
  ...
  type PatternWorkerApplyStep,
  type PatternWorkerAttachmentOperation,
  type PatternWorkerAttachmentResult,
  type PatternWorkerMatchOperation,
...
class WorkerFailure extends Error {
  readonly code: "invalid_pattern" | "unexpected_failure"

  constructor(code: "invalid_pattern" | "unexpected_failure") {
    super(code)
    this.code = code
  }
}

interface WorkerPattern extends PatternWorkerPattern {
  compiled: true
}
...
interface WorkerApplyOperationResult {
...
function validPatternSource(source: unknown): source is string {
  return (
...
function collectMatches(
  ...
  const matches: PatternWorkerSpan[] = []
...
function hasMatch(value: string, pattern: WorkerPattern): boolean {
...
function applyOperation(
  ...
  let residualPatternIndex: number | null = null
...
interface PatternWorkerApplyOperationLike {
...
function attachmentResult(
  operation: PatternWorkerAttachmentOperation,
  patterns: WorkerPattern[],
  outputs: Map<number, string>,
  budget: WorkerBudget,
): PatternWorkerAttachmentResult {
  const input =
    operation.inputOperationIndex === null
      ? operation.text
      : outputs.get(operation.inputOperationIndex)
  ...
  for (const pattern of patterns) {
    ...
    for (const match of matches) {
      const omittable =
        pattern.index === operation.omittablePatternIndex &&
...
function matchResult(
  operation: PatternWorkerMatchOperation,
  patterns: WorkerPattern[],
): PatternWorkerMatchResult {
  let firstPatternIndex: number | null = null
  for (const pattern of patterns) {
...
function processOperation(
  ...
  if (operation.kind === "match") return matchResult(operation, patterns)
  return attachmentResult(operation, patterns, outputs, budget)
}

function validRequest(value: unknown): value is PatternWorkerRequest {
  return (
    isRecord(value) &&
...
function processRequest(request: PatternWorkerRequest): PatternWorkerSuccess {
  ...
  const budget: WorkerBudget = { matchSpans: 0, outputChars: 0 }
  const results = []
...
function responseFor(request: unknown): PatternWorkerResponse {
  ...
  try {
    ...
    return {
      ...
      ok: false,
      errorCode:
        error instanceof WorkerFailure && error.code === "invalid_pattern"
          ? "invalid_pattern"
          : "unexpected_failure",
    }
  }
...
parentPort?.on("message", (request: unknown) => {
