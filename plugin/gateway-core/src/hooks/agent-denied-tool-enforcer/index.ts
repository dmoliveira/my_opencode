...
function compactDecisionText(prompt: string, description: string): string {
  return [
      ...
      if (
        ...
        if (mutationDecision.accepted && mutationDecision.char === "M") {
          ...
          if (options.decisionRuntime.config.mode === "enforce") {
            throw new Error(
              `Blocked task delegation for ${subagentType}: LLM mutation classifier marked the request as mutating for a read-only subagent. Run commit/PR/edit actions directly with the primary agent.`,
            )
          }
        }
      }
      ...
      if (violating.length === 0) {
        if (options.decisionRuntime && denied.length > 0) {
          const toolDecision = await options.decisionRuntime.decide({
            ...
            instruction: buildToolInstruction(
              denied.map((item) => String(item).toLowerCase().trim()),
              allowed.map((item) => String(item).toLowerCase().trim()),
            ),
            context: compactDecisionText(promptText, descriptionText),
            allowedChars: ["D", "A", "N"],
            decisionMeaning: {
          ...
          if (toolDecision.accepted && toolDecision.char === "D") {
            ...
            if (options.decisionRuntime.config.mode === "enforce") {
              throw new Error(
                `Blocked task delegation for ${subagentType}: LLM tool-surface classifier marked the request as implying denied tooling.${suggestion ? ` Use allowed tool '${suggestion}' instead.` : ""}`,
              )
            }
          }
        }
