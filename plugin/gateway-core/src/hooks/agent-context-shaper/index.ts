    ...
    async event(type: string, payload: unknown): Promise<void> {
      ...
      if (type === "tool.execute.before") {
        ...
        const subagentType = String(
          eventPayload.output?.args?.subagent_type ?? "",
        )
          .toLowerCase()
          .trim();
        if (!subagentType) {
        ...
        const traceId = resolveDelegationTraceId(
          eventPayload.output?.args ?? {},
        );
        annotateDelegationMetadata(
          eventPayload.output ?? {},
          eventPayload.output?.args,
        ...
        const metadata =
          loadAgentMetadata(options.directory).get(subagentType) ?? {};
        const category = String(
          eventPayload.output?.args?.category ??
      ...
      const traceId = extractDelegationTraceId(
        eventPayload.output?.args,
        eventPayload.output?.metadata,
      );
      const subagentType =
        extractDelegationSubagentType(
          eventPayload.output?.args,
