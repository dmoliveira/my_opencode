...
function parseInlineArrayValues(raw: string): string[] {
...
function expandBraceGlobs(pattern: string): string[] {
...
function parseRuleFile(path: string): RuleCandidate | null {
  let content = ""
  try {
...
function resolveTargetPath(output: ToolAfterPayload["output"]): string {
...
function matchRules(rules: RuleCandidate[], targetPath: string): RuleMatch[] {
