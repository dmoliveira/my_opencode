...
function maintenanceHelperProtectedPattern(): RegExp {
  return protectedPattern(
    String.raw`python3?\s+${SHELL_TOKEN}\s+maintenance\s+--directory\s+${SHELL_TOKEN}\s+--command\s+${SHELL_TOKEN}\s+--json`,
  )
}
