...
function sqliteProtectedPattern(): RegExp {
  return protectedPattern(
    String.raw`(?:[^\s;&|]*/)?sqlite3(?=[^;&|]*\s-readonly\b)(?:\s+${SQLITE_SAFE_FLAG})*\s+${SHELL_TOKEN}\s+(?:(?:"\.(?:tables|schema(?:\s+[^"]+)?)")|(?:'\.(?:tables|schema(?:\s+[^']+)?)')|(?:"PRAGMA\s+table_info\s*\([^";=]+\)\s*;?")|(?:'PRAGMA\s+table_info\s*\([^';=]+\)\s*;?')|(?:"SELECT\b(?![^";]*(?:load_extension|readfile|writefile|attach|pragma)\b)[^";]*;?")|(?:'SELECT\b(?![^';]*(?:load_extension|readfile|writefile|attach|pragma)\b)[^';]*;?'))`,
  )
}
