...
function isValidationCommand(command: string): boolean {
  ...
  return /\b(npm\s+(run\s+)?(test|lint|build|typecheck)|pnpm\s+(test|lint|build|typecheck)|yarn\s+(test|lint|build|typecheck)|pytest|vitest|jest|ruff\s+check|tsc\b|cargo\s+(test|clippy|check))\b/.test(
    value,
  )
}
...
export function createAdaptiveValidationSchedulerHook(options: {
  ...
  const stateBySession = new Map<string, SchedulerState>()
