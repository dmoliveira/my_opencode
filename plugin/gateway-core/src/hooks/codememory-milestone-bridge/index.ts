...
type MilestoneKind = "delegation" | "validation" | "pr_create" | "pr_merge"
type MilestoneOutcome = "passed" | "failed"
...
interface QueueItem extends Milestone {
  requestId: string
  text: string
}
...
const FAILURE_OUTPUT =
  /(^\s*\[error\]|^\s*error:|^\s*exception:|^\s*traceback\b|^\s*invalid arguments\b|^\s*unknown\s+agent\b|^\s*unknown\s+category\b|^\s*blocked delegation\b)/im
...
function callId(payload: ToolPayload): string {
  return normalize(payload.input?.callID ?? payload.input?.callId)
}

function directoryFor(payload: ToolPayload, fallback: string): string {
  return typeof payload.directory === "string" && payload.directory.trim()
    ? payload.directory
    : fallback
}

function toolName(payload: ToolPayload): string {
  return normalize(payload.input?.tool).toLowerCase()
}

function commandFromPayload(payload: ToolPayload): string {
  return normalize(payload.input?.args?.command ?? payload.output?.args?.command)
}

function exitCode(payload: ToolPayload): number | null {
...
function failureFromTaskOutput(payload: ToolPayload): boolean | null {
...
function requestId(milestone: Milestone): string {
...
function classifyMilestone(
  ...
  if (tool === "task") {
    ...
    const trace = normalize(
      payload.output?.metadata && typeof payload.output.metadata === "object"
        ? (payload.output.metadata as Record<string, unknown>).trace_id ??
            (payload.output.metadata as Record<string, unknown>).traceId
        : "",
    )
    const failed =
      type === "tool.execute.before.error" ? true : failureFromTaskOutput(payload)
    if (failed === null) {
      return null
    }
...
export function createCodememoryMilestoneBridgeHook(options: {
  ...
  function audit(reasonCode: string, item: Partial<QueueItem>): void {
  ...
  function send(item: QueueItem): Promise<void> {
    return new Promise((resolve) => {
      let settled = false
      let timeoutTimer: NodeJS.Timeout | undefined
      let terminationTimer: NodeJS.Timeout | undefined
      let timedOut = false
      const finish = (reasonCode: string): void => {
        if (settled) {
          return
        }
        settled = true
      ...
      child.once("error", () =>
        finish(timedOut ? "codememory_timeout" : "codememory_process_error"),
      )
  ...
  function drain(): void {
    ...
    void (async (): Promise<void> => {
      while (pending.size > 0) {
        const next = pending.entries().next().value as [string, QueueItem] | undefined
        if (!next) {
          break
        }
        const [id, item] = next
  ...
  function enqueue(milestone: Milestone): void {
