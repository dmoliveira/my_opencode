...
function isCommitFlow(command: string): boolean {
...
function stagedAddedLines(directory: string): string {
...
export function createSecretCommitGuardHook(options: {
  ...
  const patterns = options.patterns.map(compilePattern).filter((value): value is RegExp => value !== null)
  return {
    ...
    async event(type: string, payload: unknown): Promise<void> {
      ...
      throw new Error(
        "[secret-commit-guard] Staged diff appears to contain secrets. Remove/redact sensitive data before running git add/commit.",
      )
    },
  }
}
