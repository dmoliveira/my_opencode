...
function appendOption(values: Map<string, string[]>, name: string, value: string): void {
  const current = values.get(name) ?? []
  current.push(value)
  values.set(name, current)
}

function splitOptions(args: string[], options: Set<string>): ParsedOptions | null {
...
function hasExactlyOne(values: Map<string, string[]>, name: string): boolean {
  return (values.get(name) ?? []).length === 1
}

function validValues(parsed: ParsedOptions): boolean {
  return [...parsed.values.values()].every((values) =>
    values.every((value) => value.length <= 4096 && !/[\0\r\n]/.test(value)),
  )
}

function normalizeOcArgs(segment: string[]): string[] | null {
  if (segment[0] !== "oc") {
    return null
  }
  const args = ["oc"]
...
function parseShell(command: string): string[][] | null {
...
function optionsObject(parsed: ParsedOptions): Readonly<Record<string, readonly string[]>> {
  return Object.fromEntries(
    [...parsed.values.entries()].map(([name, values]) => [name, [...values]]),
  )
}

function matchesSandbox(parsed: ParsedOptions, sandbox: TaskerSandbox): boolean {
  return (
...
function hasKnownRecord(context: TaskerCommandContext, id: string): boolean {
...
function writeCommand(args: string[], context: TaskerCommandContext): TaskerCommandInspection | null {
  const verb = args[1] ?? ""
  if (verb === "add") {
...
function classify(segment: string[], context: TaskerCommandContext): TaskerCommandInspection | null {
...
export function inspectTaskerCommand(
  command: string,
  context: TaskerCommandContext = {},
): readonly TaskerCommandInspection[] | null {
  const segments = parseShell(command)
  if (!segments) {
    return null
  }
  const inspections = segments.map((segment) => classify(segment, context))
  return inspections.every((inspection): inspection is TaskerCommandInspection => inspection !== null)
    ? inspections
    : null
}

export function isAllowedTaskerCommand(
  command: string,
  context: TaskerCommandContext = {},
): boolean {
