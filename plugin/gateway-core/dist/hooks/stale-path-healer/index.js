...
function normalizeDirectory(directory, fallback) {
...
function maybeRewriteStaleWorktreePath(targetPath, activeRoot) {
    ...
    const rootIndex = parts.findIndex((part) => part === rootName || part.startsWith(`${rootName}-wt-`));
    if (rootIndex < 0) {
        return targetPath;
    }
...
function rewritePatchText(patchText, activeRoot) {
    if (!patchText.trim()) {
        return patchText;
    }
    return patchText.replace(/^(\*\*\*\s+(?:Add|Update|Delete)\s+File:\s+|\*\*\*\s+Move to:\s+)(.+)$/gm, (_match, prefix, pathValue) => `${prefix}${maybeRewriteStaleWorktreePath(pathValue.trim(), activeRoot)}`);
}
export function createStalePathHealerHook(options) {
    ...
    return {
        ...
        async event(type, payload) {
            ...
            const auditChanges = {};
            const baseDirectory = normalizeDirectory(typeof eventPayload.directory === "string" ? eventPayload.directory : "", activeRoot);
