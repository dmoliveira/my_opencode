...
function configuredStateDir(value, directory) {
...
function projectDigest(directory) {
    return digest(["intent-ingress-project-v1", resolve(directory)]).slice(0, 32);
}
function sourceDigest(envelope) {
    return digest([envelope.source.session_id, envelope.source.message_id]).slice(0, 20);
}
function envelopeIdentity(project, sessionId, messageId) {
...
async function assertSafeAncestorNamespace(path) {
...
async function assertSafeCreationParent(path) {
...
async function ensurePrivateDirectoryTree(path) {
    const absolute = resolve(path);
    const root = parse(absolute).root;
    const components = relative(root, absolute).split(sep).filter(Boolean);
    if (components.length === 0) {
        throw new Error("intent ingress state directory cannot be a filesystem root");
    }
    let current = root;
...
async function ensureSpool(stateDir) {
...
function sameEnvelopeIdentity(value, envelope) {
...
function assertPersistableEnvelope(envelope) {
    const candidate = envelope;
    const source = candidate.source;
    const content = candidate.content;
    const observedAt = candidate.observed_at;
    if (!hasOnlyKeys(candidate, new Set([
        "version",
        "envelope_id",
        "project_digest",
        "observed_at",
        ...
        content.char_count < 1 ||
        content.char_count > 1_048_576 ||
        typeof content.sha256 !== "string" ||
        !/^[0-9a-f]{64}$/.test(content.sha256)) {
        throw new Error("invalid intent ingress envelope");
    }
    if (envelope.envelope_id !==
        envelopeIdentity(envelope.project_digest, envelope.source.session_id, envelope.source.message_id)) {
        throw new Error("intent ingress envelope identity mismatch");
    }
    if (content.mode === "metadata" &&
    ...
    if (content.mode === "redacted_preview" &&
        (typeof content.preview !== "string" ||
            ...
            content.redacted_fields < 0 ||
            content.omitted_reason !== undefined)) {
        throw new Error("invalid intent ingress preview envelope");
    }
}
function boundedPositiveInteger(value, max, fallback) {
    return Number.isInteger(value) && value > 0 ? Math.min(value, max) : fallback;
}
async function readBoundedUtf8(handle, maxBytes) {
...
async function recoverPublishedStageLinks(finalPath, envelope, state, handle) {
    if (state.nlink === 1) {
        return true;
    }
    if (state.nlink < 1) {
    ...
    for (const stageLink of stageLinks) {
        await unlink(stageLink).catch((error) => {
            if (!isMissing(error)) {
                throw error;
            }
        });
...
async function existingEnvelopeOutcome(path, envelope, maxEnvelopeBytes) {
    let handle = null;
    try {
        handle = await open(path, constants.O_RDONLY | noFollowFlag());
    }
...
export async function persistIntentIngressEnvelope(envelope, options) {
    assertPersistableEnvelope(envelope);
    const noFollow = noFollowFlag();
    const maxEnvelopeBytes = boundedPositiveInteger(options.maxEnvelopeBytes, MAX_ENVELOPE_BYTES, 16_384);
    const softMaxPendingEntries = boundedPositiveInteger(options.softMaxPendingEntries, MAX_PENDING_ENTRIES, 1_000);
    const pending = await ensureSpool(options.stateDir);
    const finalPath = join(pending, `${envelope.envelope_id}.json`);
    const existing = await existingEnvelopeOutcome(finalPath, envelope, maxEnvelopeBytes);
    if (existing !== "missing") {
    ...
    const temporaryPath = join(pending, `${TEMP_PREFIX}${envelope.envelope_id}-${process.pid}-${randomBytes(8).toString("hex")}`);
    let handle = null;
    let published = false;
    try {
        handle = await open(temporaryPath, constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL | noFollow, PRIVATE_FILE_MODE);
        ...
        try {
            ...
            const replay = await existingEnvelopeOutcome(finalPath, envelope, maxEnvelopeBytes);
...
export function compareIntentIngressEnvelopes(left, right) {
...
function buildEnvelope(payload, options, redactor) {
    ...
    const messageId = normalizedIdentifier(payload.properties?.messageID ?? payload.output?.message?.id);
    const prompt = typeof payload.properties?.prompt === "string"
...
export function createIntentIngressOutboxHook(options) {
    ...
    return {
        ...
        async event(type, rawPayload) {
            ...
            if (!built.envelope) {
                audit(directory, built.reasonCode ?? "intent_ingress_envelope_skipped", undefined, startedAt);
                return;
            }
            if (built.redactionFailed) {
                audit(directory, "intent_ingress_redaction_failed_metadata_only", built.envelope, startedAt);
            }
            try {
                ...
                audit(directory, "intent_ingress_persist_failed", built.envelope, startedAt);
            }
        },
    };
}
