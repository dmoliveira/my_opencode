...
function resolveRuntimeConciseSkillBody(loaded) {
    ...
    const registered = CANONICAL_CONCISE_RUNTIME_CONTRACTS.get(exactPromptFingerprint([loaded.body]));
    if (registered) {
        return {
            ...loaded,
...
function buildConciseModeContext(directory, mode, candidateCacheByDirectory, bodyCacheByPath) {
    const loaded = resolveRuntimeConciseSkillBody(loadConciseSkillBody(directory, candidateCacheByDirectory, bodyCacheByPath));
    const includeModeRule = loaded.compactKind === "passthrough" || mode === "review" || mode === "commit";
    return [
    ...
    return {
        ...
        async event(type, payload) {
            ...
            const nextConcise = concise && concise.mode !== "off"
                ? buildConciseModeContext(directory, concise.mode, conciseSkillCandidateCacheByDirectory, conciseSkillBodyCacheByPath)
                : null;
