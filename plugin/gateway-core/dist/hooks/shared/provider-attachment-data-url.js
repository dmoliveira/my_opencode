...
function isBase64Alphabet(value) {
...
function isCanonicalBase64Text(value) {
...
export function isStructurallyValidPngContainer(bytes) {
    try {
        if (bytes.length < PNG_SIGNATURE.length + 12 + 12 ||
        ...
        while (offset < bytes.length) {
            if (bytes.length - offset < 12)
                return false;
            chunkCount += 1;
            if (chunkCount > MAX_OPAQUE_PNG_CHUNKS)
                return false;
            ...
            if (crcOffset < dataOffset || nextOffset > bytes.length)
                return false;
            ...
            if (pngCrc32(bytes, typeOffset, crcOffset) !== expectedCrc)
                return false;
            if (chunkCount === 1 && type !== "IHDR")
                return false;
            if (seenImageData && type !== "IDAT")
                imageDataClosed = true;
            if (type === "IHDR") {
                if (seenHeader || chunkCount !== 1 || length !== 13)
                    return false;
                ...
                if (!seenHeader || imageDataClosed || length === 0)
                    return false;
                if (colorType === 3 && !seenPalette)
                    return false;
                seenImageData = true;
                imageDataBytes += length;
                if (imageDataBytes > MAX_OPAQUE_PNG_DECODED_BYTES)
                    return false;
            }
            else if (type === "IEND") {
                return (seenHeader &&
...
export function parseCanonicalProviderAttachmentDataUrl(value, expectedMime) {
    try {
        if (!OPAQUE_PROVIDER_ATTACHMENT_MIME_TYPES.includes(expectedMime) ||
...
export function isCanonicalStructurallyValidPngDataUrl(value) {
