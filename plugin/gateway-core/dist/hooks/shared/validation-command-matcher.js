...
const LINT_COMMAND = new RegExp(`^(?:${UV_WRAPPER})?(?:eslint\\b|ruff\\s+check\\b|ruff\\s+format\\s+--check\\b|npm(?:\\s+--prefix\\s+\\S+)?\\s+run\\s+lint\\b|pnpm(?:\\s+(?:--filter\\s+\\S+)*)?\\s+(?:run\\s+)?lint\\b|yarn\\s+(?:run\\s+)?lint\\b|biome\\s+check\\b|golangci-lint\\b|cargo\\s+clippy\\b|${MAKE_VALIDATE})`, "i");
const TEST_COMMAND = new RegExp(`^(?:npm(?:\\s+--prefix\\s+\\S+)?\\s+(?:run\\s+)?test\\b|pnpm(?:\\s+(?:--filter\\s+\\S+)*)?\\s+(?:run\\s+)?test\\b|yarn\\s+(?:run\\s+)?test\\b|bun\\s+test\\b|node\\s+--test\\b|(?:npm|pnpm)\\s+exec\\s+vitest\\b|npx\\s+vitest\\b|python\\d?\\s+-m\\s+pytest\\b|python\\d?\\s+-m\\s+unittest\\b|uv\\s+run\\s+pytest\\b|pytest\\b|vitest\\b|jest\\b|go\\s+test\\b|cargo\\s+test\\b|pre-commit\\s+run\\b|make\\s+selftest\\b|make\\s+install-test\\b|python\\d?\\s+scripts/selftest\\.py\\b|\\./scripts/ci-check\\b.*\\btest[s]?\\b|${MAKE_VALIDATE})`, "i");
...
export function validationCommandDirectory(command, fallback) {
