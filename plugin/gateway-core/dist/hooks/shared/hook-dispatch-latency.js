...
function seriesKey(hookId, value) {
    return `${hookId}\u0000${value}`;
}
function percentileEstimate(series, percentile) {
...
function roundedSharePct(elapsedMs, denominatorMs) {
    if (!(denominatorMs > 0)) {
        return 0;
    }
...
export class HookDispatchLatencyCollector {
    ...
    recordSafely(input) {
        ...
        else if (measurement.completedAt - this.activeWindow.startedAt >=
            this.windowMs) {
        ...
        current.eventClassElapsedMs.set(currentEventClass, Math.min(Number.MAX_SAFE_INTEGER, (current.eventClassElapsedMs.get(currentEventClass) ?? 0) +
        ...
        series.elapsedTotalMs = Math.min(Number.MAX_SAFE_INTEGER, series.elapsedTotalMs + measurement.durationMs);
        ...
        const bucketIndex = HOOK_DISPATCH_LATENCY_BUCKETS_MS.findIndex((upperBound) => measurement.durationMs <= upperBound);
        if (bucketIndex < 0) {
    ...
    drainOne() {
        ...
        const complete = (success) => {
            ...
            if (success) {
                this.losses.detachedWindowsDropped = Math.max(0, this.losses.detachedWindowsDropped -
                    lossSnapshot.detachedWindowsDropped);
                this.losses.auditBatchesRejected = Math.max(0, this.losses.auditBatchesRejected - lossSnapshot.auditBatchesRejected);
                this.losses.auditBatchesFailed = Math.max(0, this.losses.auditBatchesFailed - lossSnapshot.auditBatchesFailed);
                this.losses.seriesSamplesDropped = Math.max(0, this.losses.seriesSamplesDropped - lossSnapshot.seriesSamplesDropped);
            }
            else {
    ...
    buildWindowRecords(window) {
        ...
        for (const [key, series] of window.series.entries()) {
            ...
            const currentEventClass = key.slice(separator + 1);
            ...
            const sharePct = roundedSharePct(elapsedTotalMs, eventClassElapsedTotalMs);
            ...
            const candidate = sharePct > LATENCY_SHARE_GATE_PCT && exceededGates.length > 0;
            built.push({
                candidate,
                hook,
...
export function createHookDispatchLatencyCollector(options) {
