...
const otelQueue = [];
const localAggregateAuditQueue = [];
...
function resolveObservabilityConfigPath(directory) {
...
function loadObservabilitySettings(directory) {
...
export function normalizeGatewayAuditSessionId(sessionId) {
...
export function gatewayAuditSessionFields(sessionId) {
...
function sanitizedAuditPayload(entry) {
...
function appendLocalAggregateAudit(directory, entry) {
    try {
        ...
        appendAuditLine(path, boundedAuditLine(sanitizedAuditPayload({ ...entry })), auditState.maxBytes, auditState.maxBackups);
        return true;
    }
    catch {
...
function drainOneLocalAggregateAuditRecord() {
    ...
    if (entry) {
        ...
        batch.nextIndex += 1;
        localAggregateAuditQueuedRecords = Math.max(0, localAggregateAuditQueuedRecords - 1);
    }
...
function scheduleLocalAggregateAuditDrain() {
...
export function enqueueGatewayLocalAggregateAudit(directory, entries, complete) {
    try {
        if (entries.length === 0) {
            return true;
        }
        ...
        const allowedEntries = entries.map((entry) => {
            const allowedEntry = Object.fromEntries(Object.entries(entry).filter(([key]) => LOCAL_AGGREGATE_AUDIT_KEYS.has(key)));
...
export function flushGatewayLocalAggregateAuditForTest() {
    localAggregateAuditGeneration += 1;
    localAggregateAuditScheduled = false;
