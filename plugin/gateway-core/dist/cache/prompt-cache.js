...
import { managedRuntimeSystemMarker, RUNTIME_SESSION_CONTEXT_MARKER, } from "../hooks/shared/stable-system-context.js";
...
function gitCommonDirectory(gitEntry, worktreeRoot) {
...
export function resolvePromptCacheScopeIdentity(directory) {
...
export function stablePromptCacheKey(input) {
...
export function exactPromptFingerprint(entries) {
    return sha256(JSON.stringify(entries)).toString("hex");
}
export function cacheableSystemPrefixObservation(system) {
    const sessionIndex = system.findIndex((entry) => managedRuntimeSystemMarker(entry) === RUNTIME_SESSION_CONTEXT_MARKER);
