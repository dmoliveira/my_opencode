...
2. Replace repeated full-table `ROW_NUMBER`/`GROUP BY` scans with six bounded queries that use correlated indexed latest-message/latest-part lookups and an indexed child-existence predicate over all sessions, including fresh children. Cap every finding query at 20 rows and compute the exact generic count separately. Do not create, migrate, or hard-code indexes in the upstream runtime database.
...
5. Compare legacy and bounded-query semantics on one owner-only compact snapshot using the same injected `now_ms`. Canonical parity excludes only scan duration and its latency warning; retain only counts, hashes, query-plan summaries, and timings in tracked evidence, then delete the compact database.
6. Benchmark six runs, discard the first, and require a warm median below the existing 1,000ms latency budget. Require all six plans to use indexed session-parent, message, and part searches with no full message/part scans or grouping. Use table-driven tests to prove arbitrary index names and valid supersets are accepted while missing or wrong-order column prefixes trigger the warned legacy fallback.
