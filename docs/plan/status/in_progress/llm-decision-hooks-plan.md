...
| Epic 5 - Prompt and protocol hardening | in_progress | Single-char contract, parser tests, prompt refinements, refusal rejection, untrusted-context serialization, stronger adversarial phrase neutralization, mixed-context coverage, hook-specific auto-slash sanitization, and successful provider/done-proof mixed-context probes are in place; remaining work is concentrated on `delegation-fallback-orchestrator` and `validation-evidence-ledger` contamination handling. |
