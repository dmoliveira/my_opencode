...
- When the helper receives an already-allowed command, it returns `NOT_EXECUTED` with mode `direct_guidance` and exit code `3`; that report is guidance, not evidence that the command ran.
