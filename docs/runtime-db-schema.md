...
Shared-memory schema migration is explicit and narrow. `/memory-lifecycle migrate --dry-run --json` inspects an existing store without creating it or changing its schema. Only a structurally exact versionless or canonical version `0` store can migrate to supported schema version `1`; use `--apply` to write the version marker in one immediate transaction. Future, malformed, missing-column, or otherwise incompatible stores fail before WAL, FTS, DDL, or data writes. A successful apply reports `changed=true` and `transaction_outcome=committed`; rerunning it is an idempotent no-op.

## Operator dashboard fields

Use `/doctor sqlite --json` for the consolidated local-store dashboard. Its four stable store keys are `runtime_history`, `session_sidecars`, `shared_memory`, and `codememory`; each preserves an owner diagnostic under `diagnostics` and provides owner-scoped `quick_fixes`. Runtime history exposes the resolved path, database/WAL footprint, scan duration/timeout, query-only verification, snapshot/scan completion, journal mode, SQLite version, JSON1 support, required-table compatibility, stale finding counts, and permission findings. Shared memory is inspected through a read-only connection and reports its schema, counts, and FTS state. Codememory is checked through `oc plan doctor` rather than direct SQLite access. A missing store is a `WARN` and is never created by the dashboard; malformed, unsafe, timed-out, or contradictory diagnostics are `FAIL`. These fields are designed for automation-safe local dashboards; do not scrape human-formatted output or publish local paths and hashes.
...
Use `/doctor sqlite --json` as the SQLite operator dashboard entry point. Use `/doctor run --json` separately for the broader plugin and workflow health surface. Alert on `WARN`/`FAIL`, schema mismatch, FTS mismatch, unsafe permissions, budget breach, and unexpected WAL growth.
...
Per-hook dispatch latency uses the existing gateway event-audit opt-in as its outer consent boundary. Its inner `hookDispatchLatency.enabled` switch defaults on but cannot activate collection while file audit is disabled, so analytics remain effectively disabled in a default plain OpenCode launch. The collector measures only hook-body elapsed time with a monotonic clock, retains one active 15-minute window plus at most two detached windows, and publishes only hook/event-class series with at least 100 samples. Fixed non-cumulative bucket upper bounds are 1, 2, 5, 10, 25, 50, 100, 250, 500, 1000, 2500, 5000, and 10000 milliseconds, plus overflow.

Persisted latency records contain aggregate sample/outcome counts, buckets, rounded elapsed sum, p50/p95/p99 bucket upper bounds, same-event-class elapsed share, fixed optimization-gate results, and bounded loss counters. They do not contain per-invocation events, runtime SQLite data, raw event types, arguments, tool/command names, paths, prompts, content, errors, model data, session identifiers, individual durations, or window identifiers/timestamps. The audit writer adds its normal publication timestamp and applies owner-only storage, bounded rotation, sanitization, and failure isolation. Latency aggregates are local-file-only and are not sent through OTLP.

Pages or processes are not a continuous measurement snapshot: plugin exit discards the active partial window and can lose queued records. Queue rejection and write-failure counters remain pending until a complete aggregate batch is acknowledged, but process exit before the next successful batch can leave loss unreported. A report with any series/window/batch loss counter is explicitly incomplete and must not justify optimization. `/gateway status --json` validates canonical hook IDs and count invariants, recomputes histogram percentiles and elapsed share from persisted aggregate inputs, reports the latest valid series from only the active audit file, omits timestamps, and caps output at 50. Stop OpenCode before deleting the active audit and rotated backups to reset history; this removes every gateway audit record, not just latency data.
