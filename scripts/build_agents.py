...
GENERATED_AGENT_MARKER_PREFIX = "<!-- Generated by scripts/build_agents.py; source: agent/specs/"
...
def _render_agent(spec: dict[str, Any], profile: str) -> tuple[str, str]:
    ...
    header_lines.append("---")
    header_lines.append(
        f"{GENERATED_AGENT_MARKER_PREFIX}{name}.json -->"
    )
...
def _is_generated_agent(path: Path) -> bool:
    """Return whether a markdown file carries the generator ownership marker."""
    try:
        return GENERATED_AGENT_MARKER_PREFIX in path.read_text(encoding="utf-8")
    except OSError:
        return False


def build_agents(
    profile: str, *, check_only: bool = False, prune_stale: bool = False
) -> int:
    ...
    stale_paths = sorted(
        ...
        if path.name not in expected_output_names
    )
    prunable_stale_paths = [path for path in stale_paths if _is_generated_agent(path)]
    if prunable_stale_paths and prune_stale and not check_only:
        for path in prunable_stale_paths:
            path.unlink()
        stale_names = ", ".join(path.name for path in prunable_stale_paths)
        print(f"agent build: pruned stale generated file(s): {stale_names}")
    if check_only:
        if stale_paths:
            changed += len(stale_paths)
            stale_names = ", ".join(path.name for path in stale_paths)
            print(f"agent build check: stale generated file(s): {stale_names}")
        if changed:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--prune-stale",
        action="store_true",
        help="remove top-level agent markdown files without a matching spec",
    )
